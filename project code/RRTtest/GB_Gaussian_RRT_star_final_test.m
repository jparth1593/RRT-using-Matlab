...
while i <= iterations
    ...
    [x_rand,y_rand,eps,eps1] = samplepoint(height,width,cmax,costideal,slop,qcenter,eps,eps1);
  ...
  %  y_rand = height*rand();
   ...
   %optimization in edge developement
    ...
    if goalcost <= eps
        ...
        optimumtolarance =  ((906.225*4)/100)+906.225;
 ...
 %   hold on
 %   drawnow
%   line(soltree(:,1),soltree(:,2),'LineWidth',1,'Color','g');
